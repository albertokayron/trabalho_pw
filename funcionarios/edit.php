...
try {
    ...
    if (!$func) {
        $_SESSION['toast'] = ['type' => 'error', 'message' => 'Funcionário não encontrado.'];
        header("Location: read.php");
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nome) || empty($funcao) || empty($data_admissao) || empty($data_nascimento) || empty($salario)) {
        ...
        if ($birthDate >= $today) {
            ...
            try {
                $stmt = $conn->prepare("UPDATE funcionarios SET nome = :nome, funcao = :funcao, data_admissao = :data_admissao, data_nascimento = :data_nascimento, salario = :salario WHERE id = :id");
                $stmt->execute([
                    'nome' => $nome,
                    'funcao' => $funcao,
                ...
                $error_message = 'Erro ao atualizar dados do funcionário: ' . $e->getMessage();
            }
        }
    }
}
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div>
        ...
        <h1 class="text-2xl font-bold tracking-tight text-white">Editar Cadastro do Colaborador</h1>
        <p class="text-sm text-zinc-400">Modifique as informações profissionais ou dados pessoais do funcionário.</p>
    </div>
    ...
    <div class="bg-zinc-900 border border-zinc-800 rounded-xl p-6 md:p-8 shadow-xl">
        ...
        <form action="edit.php?id=<?php echo $func_id; ?>" method="POST" class="space-y-6">
